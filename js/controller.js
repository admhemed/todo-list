(function (window) {
	...
	function Controller(model, view) {
		...
		that.view.bind('itemEditDone', function (item) {
			that.editItemSave(item.id, item.title, item.state);
		});
		...
		that.view.bind('itemComplete', function (item) {
			that.editItemSaveComplete(item.id, item.title, item.state);
		});
		that.view.bind('clearComplete', function (item) {
			// that.editItemSaveComplete(item.id, item.title, item.state);
		});
	 ...
	 */
	Controller.prototype.editItemSave = function (id, title, state) {
		var that = this;
		if (title.trim()) {
			that.model.update(id, {title: title, state: state}, function () {
				that.view.render('editItemDone', {id: id, title: title, state: state});
			});
		} else {
			that.removeItem(id);
		}
	};
	Controller.prototype.editItemSaveComplete = function (id, title, state) {
		var that = this;
		that.model.update(id, {title: title, state: state}, function () {
			that.view.render('complete', {id: id, title: title, state: state});
		});
